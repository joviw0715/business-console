...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  console.log(`[resume] campaign ${id} — querying pending contacts`);

  const { rows: contacts } = await pool.query(
    "SELECT id, phone FROM contacts WHERE campaign_id = $1 AND status = 'pending'",
    [id],
  );
  console.log(`[resume] campaign ${id} — found ${contacts.length} pending contacts`);
  ...
  for (const contact of contacts) {
    ...
    console.log(`[resume] enqueued contact ${contact.id} (${contact.phone})`);
  }

  await pool.query("UPDATE campaigns SET status = 'running' WHERE id = $1", [id]);
  console.log(`[resume] campaign ${id} set to running, enqueued ${contacts.length} jobs`);
