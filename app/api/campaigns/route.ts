...
export async function POST(req: Request) {
  ...
  const { name, description, system_prompt, voice_id, max_retries, call_timeout_sec, greeting_text, webhook_url, scheduled_at } = body;
  ...
  await pool.query(
    `INSERT INTO campaign_config (campaign_id, system_prompt, voice_id, max_retries, call_timeout_sec, greeting_text, webhook_url)
    ...
    [id, system_prompt ?? '', voice_id ?? 'Cantonese_GentleLady', max_retries ?? 2, call_timeout_sec ?? 60, greeting_text ?? '', webhook_url ?? null],
  );
